...
const imgFolderName = ['標準版', '娛樂版', '體育版', '未來版'];
...
function ResizeIcon() {
	...
	function handleSave() {
		...
		for (let i = 0; i < imgSize.length; i++) {
			...
			if (bothVersion) {
				for (let j = 0; j < 4; j++) {
					let imgFolder = zip.folder(imgFolderName[j]);
					if (j === 0) {
					...
					if (j === 3) {
						...
						if (name.includes(webId)) {
							name = document
								.getElementById(`iconName${i + 1}`)
								.textContent.replace('app_icon_', 'app_icon_future_');
						}
					}
					imgFolder.file(name, src, { base64: true });
		...
		zip.generateAsync({ type: 'blob' }).then(function (content) {
			if (bothVersion) {
				saveAs(content, `${webId}站點 標準版、娛樂版、體育版、未來版APP ICON`);
				return;
			}
			if (valueVersion === 1) {
			...
			if (valueVersion === 4) {
				saveAs(content, `${webId}站點 未來版APP ICON`);
			}
		});
	}
